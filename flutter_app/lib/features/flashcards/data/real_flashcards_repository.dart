import 'package:dio/dio.dart';
import 'package:social_study_app/features/flashcards/data/demo_flashcards_repository.dart'
    show
        FlashcardGenerationUnavailableException,
        FlashcardNotFoundException,
        FlashcardNotRatableException,
        NoFlashcardTopicsException;
import 'package:social_study_app/features/flashcards/data/flashcards_repository.dart';
import 'package:social_study_app/shared/models/flashcard.dart';

/// Dio-backed implementation hitting Sprint 3.12's FastAPI routes.
///
/// Error translation maps HTTP statuses to the typed exceptions defined
/// alongside the demo implementation. The Dio error interceptor (see
/// [DioClient]) already produces friendly text for transport-level
/// failures; we add type information so the UI can branch on specific
/// terminal states (e.g. "show admin-needed banner" on 409, "back off
/// and retry" on 503).
class RealFlashcardsRepository implements FlashcardsRepository {
  RealFlashcardsRepository({required this.dio});

  final Dio dio;

  static const _apiPrefix = '/api/v1';

  @override
  Future<Flashcard> next({required String workspaceId}) async {
    try {
      final response = await dio.post<Map<String, dynamic>>(
        '$_apiPrefix/workspaces/$workspaceId/flashcards/next',
      );
      return Flashcard.fromJson(response.data!);
    } on DioException catch (e) {
      throw _translateNext(e);
    }
  }

  @override
  Future<FlashcardRatingResponse> rate({
    required String workspaceId,
    required String flashcardId,
    required FlashcardRatingSubmission submission,
  }) async {
    try {
      final response = await dio.post<Map<String, dynamic>>(
        '$_apiPrefix/workspaces/$workspaceId/flashcards/$flashcardId/rate',
        data: submission.toJson(),
      );
      return FlashcardRatingResponse.fromJson(response.data!);
    } on DioException catch (e) {
      throw _translateRate(e);
    }
  }

  // ── Error translation ───────────────────────────────────────────────────

  Exception _translateNext(DioException e) {
    final status = e.response?.statusCode;
    if (status == 409) {
      return NoFlashcardTopicsException(
        _detailMessage(e.response?.data) ?? 'No topics available',
      );
    }
    if (status == 503) {
      return FlashcardGenerationUnavailableException(
        message: _detailMessage(e.response?.data) ??
            'Couldn\'t generate a flashcard right now',
        retryAfterSeconds: _retryAfterSeconds(e.response?.headers),
      );
    }
    return e;
  }

  Exception _translateRate(DioException e) {
    final status = e.response?.statusCode;
    if (status == 404) {
      return FlashcardNotFoundException(
        _detailMessage(e.response?.data) ?? 'Flashcard not found',
      );
    }
    if (status == 409) {
      return FlashcardNotRatableException(
        _detailMessage(e.response?.data) ??
            'Flashcard is not in a ratable state',
      );
    }
    return e;
  }

  String? _detailMessage(Object? body) {
    if (body is Map<String, dynamic>) {
      final detail = body['detail'];
      if (detail is String) return detail;
    }
    return null;
  }

  int _retryAfterSeconds(Headers? headers) {
    if (headers == null) return 30;
    final value = headers.value('retry-after');
    if (value == null) return 30;
    return int.tryParse(value) ?? 30;
  }
}
