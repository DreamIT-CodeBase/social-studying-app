...
def test_adjacent_chunks_overlap_by_configured_amount():
    ...
    chunks = text_chunker.chunk_text(text, target_chars=300, overlap_chars=50, min_chars=50)
    assert len(chunks) >= 2
...
def test_falls_back_to_sentence_boundary_when_no_paragraph():
    ...
    chunks = text_chunker.chunk_text(sentences, target_chars=100, overlap_chars=20, min_chars=20)
...
def test_falls_back_to_word_boundary_when_no_sentence():
    ...
    chunks = text_chunker.chunk_text(text, target_chars=50, overlap_chars=10, min_chars=10)
...
def test_hard_cut_when_no_whitespace_in_window():
    ...
    chunks = text_chunker.chunk_text(text, target_chars=500, overlap_chars=50, min_chars=100)
...
def test_small_tail_merges_into_previous_chunk():
    ...
    chunks = text_chunker.chunk_text(text, target_chars=100, overlap_chars=20, min_chars=30)
...
def test_chunker_always_makes_progress_even_with_tiny_overlap():
    ...
    text = "x" * 1000
    chunks = text_chunker.chunk_text(text, target_chars=100, overlap_chars=99, min_chars=10)
